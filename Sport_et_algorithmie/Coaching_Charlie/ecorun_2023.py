...
# Charger le fichier GPX de 2023 :
with open("ecorun_2023_copie.gpx", "r") as gpx_file:
        ...
        for point in segment.points:
            ...
            if point.extensions:
                for extension in point.extensions:
                    ...
                    atemp = extension.find("{http://www.garmin.com/xmlschemas/TrackPointExtension/v1}atemp")
                    ...
                    liste_atemp.append(atemp.text)
                    liste_hr.append(int(hr.text))
                    liste_cad.append(cad.text)
...
df = pd.read_excel('Résultats_2023_scrape.xlsx')
...
for index, row in df.iterrows():
    ...
    liste_rang.append(str(valeurs[0]))
    liste_coureurs.append(str(valeurs[2]))
    sexe = str(valeurs[3])
    liste_sexe.append(sexe[0])
    cat = str(valeurs[4])
    liste_cat.append(cat[:2])
    #liste_rang_cat.append(valeurs[4][])
    liste_club.append(str(valeurs[5]))
    temps = str(valeurs[6])
    liste_temps.append(temps[:8])
    liste_pace.append(str(valeurs[7]))

# Mise en forme des listes pour appliquer le même traitement que pour 2022 :
liste_rang.pop(0)
liste_coureurs.pop(0)
liste_sexe.pop(0)
for i in range(len(liste_sexe)) :
    liste_sexe[i] = liste_sexe[i]+'.'
    liste_rang[i] = float(liste_rang[i])
    liste_rang[i] = int(liste_rang[i])
liste_cat.pop(0)
liste_club.pop(0)
liste_temps.pop(0)
liste_pace.pop(0)
...
souhait_2 = input("Voulez-vous afficher le dashboard de l'édition 2023 ? (oui/non) : ")

# Affichage des différents graphiques :
if souhait_2 == 'oui' :
    def affiche() :
        '''Affiche le graphique de cote = f(rang)
        ...
        plt.figure('Allures de la course en 2023')
        ...
        plt.title('Profil de la course 2023 et Cadence')
        ax2 = ax1.twinx()
        ...
        # Graphique de la fc :
        fig, ax1 = plt.subplots()
        plt.title('Profil de la course 2023 et HR')
        ax2 = ax1.twinx()
        ...
        plt.figure('Physionomie de la course en 2023')
        plt.title('Physionomie de la course en 2023')
        ...
        plt.legend()
        plt.xlabel('Rang allant de 1 (tout à gauche) à 404 (tout à droite)')
        plt.xlim(-5, 409)
