...
class Actions:
    ...
    def talk(game, list_of_words,number_of_parameters):
        ...
        if character is None:
            print(f"'{character_name}' n'est pas dans la salle.")
            return False
        msg = character.get_msg(character_name)
        if msg:
            print(f"{character_name} dit : {msg}")
        return True
    ...
    def unlock(game, list_of_words,number_of_parameters):
        ...
        current_room = game.player.current_room
        if current_room.name == "Interieur":
            if code == "6295":
                print("Vous avez gagné !!! Merci d'avoir jouer !")
                return True
            else:
                print("Vous avez perdu ! Merci d'avoir jouer")
                return False
        else:
            print("Il n'y a pas de code à dévérouiller ici !")
            return False
